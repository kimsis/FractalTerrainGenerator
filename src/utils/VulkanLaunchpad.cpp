...
// One per swapchain image (not per frame-in-flight!): a renderFinishedSemaphore is waited on
// by vkQueuePresentKHR, whose consumption is tracked by the presentation engine, not by
// mSyncHostWithDeviceFence. Indexing it by frame-in-flight instead of swapchain image allowed
// the same semaphore to be re-signaled while still in use by an outstanding present operation
...
bool vklInitFramework(VkInstance vk_instance, VkSurfaceKHR vk_surface, VkPhysicalDevice vk_physical_device, VkDevice vk_device, VkQueue vk_queue, const VklSwapchainConfig& swapchain_config)
{
	...
	// One renderFinishedSemaphore per swapchain image
...
void vklPresentCurrentSwapchainImage()
{
	...
	mQueue.submit({ vk::SubmitInfo()
		...
		.setPSignalSemaphores(&mRenderFinishedSemaphores[mCurrentSwapChainImageIndex].get())
	...
	auto presentInfo = vk::PresentInfoKHR()
		.setWaitSemaphoreCount(1u)
		.setPWaitSemaphores(&mRenderFinishedSemaphores[mCurrentSwapChainImageIndex].get())
